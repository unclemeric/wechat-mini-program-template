...
import { wxAlert, wxConfirm } from '../../utils'
import './index.less'
...
export default {
  ...
  setup(prop, _context) {
    // const prefix = api.img_prefix
    // let imagePrefix = ['.BMP', '.JPG', '.JPEG', '.PNG', '.GIF']
    const abortList: Array<Number> = []
    // const uploadedFile: Array<String> = []
    const list: Array<UploadedFile> = prop.fileList.map((file) => {
      return {
        path: file.path,
        state: 1, // 待上传
        progress: 100,
    ...
    // const list: Array<UploadedFile> = prop.fileList.map(
    //   (filePath: string): UploadedFile => {
    //     let i = filePath.lastIndexOf('.')
    //     let prefix = filePath.substring(i, filePath.length)
    //     let obj: UploadedFile = {
    //       path: filePath,
    //       state: 1, // 待上传
    //       progress: 100,
    //       msg: '上传完成',
    //       type: imagePrefix.includes(prefix.toUpperCase()) ? 'image' : 'video',
    //     }
    //     return obj
    //   }
    // )
    ...
    const chooseImage = () => {
      ...
      Taro.chooseMedia({
        count: state.maximum - state.list.length,
